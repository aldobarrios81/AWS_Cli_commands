#!/bin/bash
# verify-network-gateway-changes-monitoring.sh
# Verificar configuración de monitoring para cambios en Network Gateways
# Regla de seguridad CIS AWS: 3.12 - Monitor network gateway changes
# Uso: ./verify-network-gateway-changes-monitoring.sh [perfil]
...
FILTER_NAME="CIS-NetworkGatewayChanges"
ALARM_PREFIX="CIS-3.12-NetworkGatewayChanges"
...
echo "=================================================================="
echo -e "${BLUE}🔍 VERIFICANDO CIS 3.12 - NETWORK GATEWAY CHANGES MONITORING${NC}"
echo "=================================================================="
echo -e "Perfil: ${GREEN}$PROFILE${NC} | Región: ${GREEN}$REGION${NC}"
echo "Verificando configuración de monitoreo para cambios en Network Gateways"
echo ""
...
# Verificar Network Gateways en la cuenta
echo -e "${PURPLE}=== Verificando Network Gateways en la cuenta ===${NC}"

# Contar y analizar Internet Gateways
IGW_COUNT=$(aws ec2 describe-internet-gateways --profile "$PROFILE" --region "$REGION" --query 'length(InternetGateways)' --output text 2>/dev/null)

if [ -z "$IGW_COUNT" ] || [ "$IGW_COUNT" -eq 0 ]; then
    echo -e "🌐 Internet Gateways: ${YELLOW}0 gateways${NC}"
else
    echo -e "🌐 Internet Gateways encontrados: ${GREEN}$IGW_COUNT gateways${NC}"
    
    # Mostrar detalles de IGWs
    echo -e "${BLUE}📊 Detalles de Internet Gateways:${NC}"
    aws ec2 describe-internet-gateways --profile "$PROFILE" --region "$REGION" --query 'InternetGateways[*].[InternetGatewayId,State,Attachments[0].VpcId,Attachments[0].State]' --output table 2>/dev/null
    
    # Contar IGWs por estado
    ATTACHED_IGW=$(aws ec2 describe-internet-gateways --profile "$PROFILE" --region "$REGION" --query 'length(InternetGateways[?Attachments[0].State==`attached`])' --output text 2>/dev/null)
    DETACHED_IGW=$(aws ec2 describe-internet-gateways --profile "$PROFILE" --region "$REGION" --query 'length(InternetGateways[?length(Attachments)==`0`])' --output text 2>/dev/null)
    
    echo -e "   📌 IGWs Asociados: ${GREEN}$ATTACHED_IGW${NC} | IGWs Libres: ${YELLOW}$DETACHED_IGW${NC}"
fi

echo ""

# Contar y analizar VPN Gateways
VGW_COUNT=$(aws ec2 describe-vpn-gateways --profile "$PROFILE" --region "$REGION" --query 'length(VpnGateways)' --output text 2>/dev/null)

if [ -z "$VGW_COUNT" ] || [ "$VGW_COUNT" -eq 0 ]; then
    echo -e "🔐 VPN Gateways: ${YELLOW}0 gateways${NC}"
else
    echo -e "🔐 VPN Gateways encontrados: ${GREEN}$VGW_COUNT gateways${NC}"
    
    # Mostrar detalles de VGWs
    echo -e "${BLUE}📊 Detalles de VPN Gateways:${NC}"
    ...
    # Contar VGWs por estado
    AVAILABLE_VGW=$(aws ec2 describe-vpn-gateways --profile "$PROFILE" --region "$REGION" --query 'length(VpnGateways[?State==`available`])' --output text 2>/dev/null)
    
    echo -e "   📌 VGWs Disponibles: ${GREEN}$AVAILABLE_VGW${NC}"
fi

echo ""

# Contar y analizar Customer Gateways
CGW_COUNT=$(aws ec2 describe-customer-gateways --profile "$PROFILE" --region "$REGION" --query 'length(CustomerGateways)' --output text 2>/dev/null)

if [ -z "$CGW_COUNT" ] || [ "$CGW_COUNT" -eq 0 ]; then
    echo -e "🏢 Customer Gateways: ${YELLOW}0 gateways${NC}"
else
    echo -e "🏢 Customer Gateways encontrados: ${GREEN}$CGW_COUNT gateways${NC}"
    
    # Mostrar detalles de CGWs
    echo -e "${BLUE}📊 Detalles de Customer Gateways:${NC}"
    aws ec2 describe-customer-gateways --profile "$PROFILE" --region "$REGION" --query 'CustomerGateways[*].[CustomerGatewayId,State,IpAddress,Type]' --output table 2>/dev/null
    
    # Contar CGWs por estado
    AVAILABLE_CGW=$(aws ec2 describe-customer-gateways --profile "$PROFILE" --region "$REGION" --query 'length(CustomerGateways[?State==`available`])' --output text 2>/dev/null)
    
    echo -e "   📌 CGWs Disponibles: ${GREEN}$AVAILABLE_CGW${NC}"
fi

# Total de gateways
TOTAL_GATEWAYS=$((IGW_COUNT + VGW_COUNT + CGW_COUNT))
echo -e "${GREEN}📊 Total de Network Gateways en la cuenta: $TOTAL_GATEWAYS${NC}"
...
if [ -z "$ALARMS" ]; then
    echo -e "${RED}❌ No se encontraron CloudWatch Alarms para CIS 3.12${NC}"
else
...
if [ "$TOTAL_GATEWAYS" -gt 0 ]; then
    echo -e "✅ Network Gateways: ${GREEN}$TOTAL_GATEWAYS GATEWAYS MONITOREADOS${NC}"
    echo -e "   📌 Internet GWs: ${GREEN}$IGW_COUNT${NC} | VPN GWs: ${GREEN}$VGW_COUNT${NC} | Customer GWs: ${GREEN}$CGW_COUNT${NC}"
else
    echo -e "⚠️ Network Gateways: ${YELLOW}NO ENCONTRADOS${NC}"
fi
...
if [ $FILTERS_FOUND -gt 0 ] && [ $ALARMS_FOUND -gt 0 ] && [ -n "$SNS_TOPIC_ARN" ]; then
    echo -e "${GREEN}🎉 CIS 3.12 - CONFIGURACIÓN COMPLETA Y FUNCIONAL${NC}"
    echo -e "${BLUE}💡 Network Gateway changes monitoring está activo${NC}"
else
    echo -e "${YELLOW}⚠️ CONFIGURACIÓN INCOMPLETA${NC}"
    echo -e "${BLUE}💡 Ejecuta el script de configuración para completar CIS 3.12${NC}"
fi
...
echo "3. Establecer procedimientos de respuesta a cambios de Gateway"
echo "4. Revisar asociaciones de gateways a VPCs críticas"
echo "5. Documentar la configuración para el equipo de seguridad"
echo "6. Programar auditorías regulares de Network Gateways"