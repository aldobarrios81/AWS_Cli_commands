#!/bin/bash
# setup-route-table-changes-monitoring.sh
# Create log metric filter and alarm for route table changes
# Regla de seguridad CIS AWS: 3.13 - Monitor route table changes
# Uso: ./setup-route-table-changes-monitoring.sh [perfil]
...
ALARM_DESCRIPTION="CIS 3.13 - Route table changes detected"
NOTIFICATION_EMAIL="felipe.castillo@azlogica.com"

# Patrón del filtro para detectar cambios en route tables
...
echo "=================================================================="
echo -e "${BLUE}🔒 IMPLEMENTANDO CIS 3.13 - ROUTE TABLE CHANGES MONITORING${NC}"
echo "=================================================================="
echo -e "Perfil: ${GREEN}$PROFILE${NC} | Región: ${GREEN}$REGION${NC}"
echo "Regla: Create log metric filter and alarm for route table changes"
echo ""
...
# Paso 2: Verificar Route Tables existentes
echo -e "${PURPLE}=== Paso 2: Verificando Route Tables existentes ===${NC}"
...
if [ -z "$RT_COUNT" ] || [ "$RT_COUNT" -eq 0 ]; then
    echo -e "${YELLOW}⚠️ No se encontraron Route Tables en la cuenta${NC}"
else
    echo -e "✅ Route Tables encontradas en la cuenta: ${GREEN}$RT_COUNT tablas${NC}"
    echo -e "📋 Estas tablas de enrutamiento serán monitoreadas por cambios"
    
    # Mostrar ejemplos de Route Tables (primeros 3)
    echo -e "${BLUE}📄 Ejemplos de Route Tables (primeros 3):${NC}"
    aws ec2 describe-route-tables --profile "$PROFILE" --region "$REGION" --query 'RouteTables[:3].[RouteTableId,VpcId,length(Routes)]' --output table 2>/dev/null | head -10
    
    # Verificar VPCs con Route Tables
    echo -e "${BLUE}🌐 VPCs con Route Tables:${NC}"
    ...
    echo -e "${BLUE}⚠️ Verificando rutas críticas:${NC}"
    
    # Buscar rutas 0.0.0.0/0 (rutas por defecto)
    DEFAULT_ROUTES=$(aws ec2 describe-route-tables --profile "$PROFILE" --region "$REGION" --query 'RouteTables[?Routes[?DestinationCidrBlock==`0.0.0.0/0`]]' --output json 2>/dev/null | jq '. | length' 2>/dev/null)
    
    if [ -n "$DEFAULT_ROUTES" ] && [ "$DEFAULT_ROUTES" -gt 0 ]; then
        echo -e "   📊 Rutas por defecto (0.0.0.0/0) encontradas: ${YELLOW}$DEFAULT_ROUTES${NC}"
        echo -e "   ${BLUE}💡 Estas rutas controlan el tráfico hacia Internet${NC}"
    else
        echo -e "   📊 No se encontraron rutas por defecto específicas"
    fi
    ...
    echo -e "   📊 Total de rutas en todas las tablas: ${GREEN}$TOTAL_ROUTES rutas${NC}"
fi
...
# Resumen final
echo "=================================================================="
echo -e "${GREEN}🎉 IMPLEMENTACIÓN CIS 3.13 COMPLETADA${NC}"
echo "=================================================================="
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "✅ Metric Filters para detectar cambios en Route Tables"
echo "✅ CloudWatch Alarms para alertas de modificaciones de enrutamiento"
echo "✅ SNS Topic para notificaciones inmediatas de seguridad"
echo "✅ Cumplimiento con CIS AWS Benchmark 3.13"
echo ""

echo -e "${BLUE}🔍 EVENTOS ROUTE TABLE MONITOREADOS:${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📋 GESTIÓN DE TABLAS:"
echo "• CreateRouteTable - Creación de nuevas tablas de enrutamiento"
echo "• DeleteRouteTable - Eliminación de tablas de enrutamiento"
echo ""
echo "🛣️ GESTIÓN DE RUTAS:"
...
echo ""
echo "🔗 ASOCIACIONES:"
echo "• ReplaceRouteTableAssociation - Cambios de asociación tabla-subnet"
echo "• DisassociateRouteTable - Desasociación de tablas"
echo ""

echo -e "${RED}⚠️ IMPORTANCIA DE ESTA REGLA:${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "• Route Tables definen cómo se enruta el tráfico en las VPCs"
echo "• Controlan el acceso a Internet, servicios AWS y redes corporativas"
echo "• Cambios no autorizados pueden redirigir tráfico sensible"
echo "• Rutas maliciosas pueden interceptar comunicaciones"
echo "• Eliminación accidental puede causar pérdida de conectividad"
echo "• Modificaciones pueden exponer subredes privadas"
echo "• Detección temprana previene man-in-the-middle attacks"
echo ""
...
echo -e "   ${BLUE}./verify-route-table-changes-monitoring.sh${NC}"
echo ""
echo "3. Probar las notificaciones (opcional):"
echo -e "   ${BLUE}aws sns publish --topic-arn $SNS_TOPIC_ARN --message 'Prueba de notificación CIS 3.13' --profile $PROFILE${NC}"
echo ""
echo "4. Establecer procedimientos de respuesta para cambios de Route Table:"
echo "   - Investigar inmediatamente cambios no autorizados en enrutamiento"
echo "   - Revisar nuevas rutas hacia destinos externos (0.0.0.0/0)"
echo "   - Validar modificaciones de rutas críticas con el equipo de red"
echo "   - Auditar cambios de asociaciones subnet-route table"
echo "   - Verificar eliminación de rutas que puedan interrumpir servicios"
echo ""
echo "5. Monitorear regularmente las alertas y patrones de enrutamiento"
echo ""
