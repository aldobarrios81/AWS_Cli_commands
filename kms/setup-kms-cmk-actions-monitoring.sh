#!/bin/bash
# setup-kms-cmk-actions-monitoring.sh
# Create log metric filter and alarm for KMS CMK Actions
# Regla de seguridad CIS AWS: 3.7 - Monitor KMS Customer Master Key (CMK) actions
...
ALARM_NAME="CIS-3.7-KMSCMKActions"
SNS_TOPIC_NAME="cis-security-alerts"
ALARM_DESCRIPTION="CIS 3.7 - KMS Customer Master Key actions detected"
NOTIFICATION_EMAIL="felipe.castillo@azlogica.com"

# Patrón del filtro para detectar acciones críticas en KMS CMK
FILTER_PATTERN='{ ($.eventSource = "kms.amazonaws.com") && (($.eventName = "CreateKey") || ($.eventName = "ScheduleKeyDeletion") || ($.eventName = "DisableKey") || ($.eventName = "EnableKey") || ($.eventName = "PutKeyPolicy") || ($.eventName = "CreateAlias") || ($.eventName = "DeleteAlias") || ($.eventName = "UpdateAlias")) }'
...
echo "=================================================================="
echo -e "${BLUE}🔒 IMPLEMENTANDO CIS 3.7 - KMS CMK ACTIONS MONITORING${NC}"
echo "=================================================================="
echo "Perfil: $PROFILE | Región: $REGION"
echo "Regla: Create log metric filter and alarm for KMS CMK Actions"
echo ""
...
if [ $? -ne 0 ]; then
    ...
    echo "   - sns:ListTopics"
    echo "   - kms:ListKeys (para verificar KMS keys)"
    echo ""
    exit 1
fi
...
# Paso 2: Verificar KMS Keys (opcional pero informativo)
echo "=== Paso 2: Verificando KMS Keys existentes ==="

KMS_KEYS=$(aws kms list-keys \
    --profile "$PROFILE" --region "$REGION" \
    --query 'Keys[].{KeyId:KeyId}' \
    --output text 2>/dev/null | wc -l)

if [ $? -eq 0 ] && [ "$KMS_KEYS" -gt 0 ]; then
    echo -e "${GREEN}✅ KMS Keys encontradas en la cuenta: $KMS_KEYS keys${NC}"
    echo -e "${BLUE}📋 Estas keys serán monitoreadas por cambios administrativos${NC}"
else
    echo -e "${YELLOW}⚠️ No se encontraron KMS Keys o sin permisos para listarlas${NC}"
    echo -e "${BLUE}💡 El monitoreo funcionará cuando se creen o modifiquen KMS Keys${NC}"
fi
echo ""
...
# Resumen final
echo "=================================================================="
echo -e "${GREEN}🎉 IMPLEMENTACIÓN CIS 3.7 COMPLETADA${NC}"
echo "=================================================================="
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "✅ Metric Filters para detectar acciones críticas en KMS CMK"
echo "✅ CloudWatch Alarms para alertas de actividades de cifrado"
...
echo "• DisableKey - Desactivación de keys de cifrado"
echo "• EnableKey - Reactivación de keys de cifrado"
echo "• PutKeyPolicy - Modificación de políticas de keys"
echo "• CreateAlias - Creación de alias para keys"
echo "• DeleteAlias - Eliminación de alias de keys"
echo "• UpdateAlias - Actualización de alias de keys"
echo ""
echo -e "${YELLOW}⚠️ IMPORTANCIA DE ESTA REGLA:${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "• KMS Keys son críticas para el cifrado de datos sensibles"
echo "• Cambios no autorizados pueden comprometer la seguridad de datos"
echo "• Eliminación accidental puede causar pérdida permanente de datos"
echo "• Modificaciones de políticas afectan el control de acceso"
echo "• Detección temprana previene exposición de datos cifrados"
echo ""
...
echo "   ./verify-kms-cmk-actions-monitoring.sh"
echo ""
echo "3. Probar las notificaciones (opcional):"
echo "   aws sns publish --topic-arn $SNS_TOPIC_ARN --message 'Prueba de notificación CIS 3.7' --profile $PROFILE"
echo ""
echo "4. Establecer procedimientos de respuesta para eventos KMS:"
echo "   - Investigar inmediatamente eliminaciones no programadas"
echo "   - Revisar cambios de políticas de keys críticas"
echo "   - Validar creación de nuevas keys con el equipo de seguridad"
echo ""
echo "5. Monitorear regularmente las alertas y patrones de uso"
echo ""
echo "6. Documentar esta configuración en la política de cifrado"
echo ""