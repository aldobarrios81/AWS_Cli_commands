#!/bin/bash
# verify-security-group-changes-monitoring.sh
# Verificar configuración de monitoring para cambios en Security Groups
# Regla de seguridad CIS AWS: 3.10 - Monitor security group changes
# Uso: ./verify-security-group-changes-monitoring.sh [perfil]
...
ALARM_PREFIX="CIS-3.10-SecurityGroupChanges"
...
echo "=================================================================="
echo -e "${BLUE}🔍 VERIFICANDO CIS 3.10 - SECURITY GROUP CHANGES MONITORING${NC}"
echo "=================================================================="
echo -e "Perfil: ${GREEN}$PROFILE${NC} | Región: ${GREEN}$REGION${NC}"
echo "Verificando configuración de monitoreo para cambios en Security Groups"
echo ""
...
# Verificar Security Groups en la cuenta
echo -e "${PURPLE}=== Verificando Security Groups en la cuenta ===${NC}"

# Contar y listar security groups
...
if [ -z "$SG_COUNT" ] || [ "$SG_COUNT" -eq 0 ]; then
    echo -e "${YELLOW}⚠️ No se encontraron Security Groups${NC}"
else
    echo -e "${GREEN}✅ Security Groups encontrados: $SG_COUNT grupos${NC}"
    
    # Mostrar estadísticas de security groups
    echo -e "${BLUE}📊 Estadísticas de Security Groups:${NC}"
    
    # Security Groups por VPC
    echo -e "${BLUE}📄 Security Groups por VPC:${NC}"
    aws ec2 describe-security-groups --profile "$PROFILE" --region "$REGION" --query 'SecurityGroups[*].[VpcId]' --output text | sort | uniq -c | while read count vpc_id; do
        if [ "$vpc_id" == "None" ] || [ -z "$vpc_id" ]; then
            echo -e "   📄 EC2-Classic: ${GREEN}$count grupos${NC}"
        else
            VPC_NAME=$(aws ec2 describe-vpcs --vpc-ids "$vpc_id" --profile "$PROFILE" --region "$REGION" --query 'Vpcs[0].Tags[?Key==`Name`].Value' --output text 2>/dev/null)
            if [ -z "$VPC_NAME" ] || [ "$VPC_NAME" == "None" ]; then
                VPC_NAME="Sin nombre"
            fi
            echo -e "   📄 VPC $vpc_id ($VPC_NAME): ${GREEN}$count grupos${NC}"
        fi
    done
    
    echo ""
    
    # Ejemplos de Security Groups críticos
    echo -e "${BLUE}🔒 Ejemplos de Security Groups (primeros 5):${NC}"
    aws ec2 describe-security-groups --profile "$PROFILE" --region "$REGION" --query 'SecurityGroups[:5].[GroupId,GroupName,Description]' --output table 2>/dev/null
    
    echo ""
    
    # Verificar reglas potencialmente peligrosas
    echo -e "${BLUE}⚠️ Verificando reglas de seguridad potencialmente peligrosas:${NC}"
    
    # Buscar reglas que permiten acceso desde cualquier IP (0.0.0.0/0)
    OPEN_RULES=$(aws ec2 describe-security-groups --profile "$PROFILE" --region "$REGION" --query 'SecurityGroups[?length(IpPermissions[?IpRanges[?contains(CidrIp, `0.0.0.0/0`)]])>`0`].[GroupId,GroupName]' --output text 2>/dev/null | wc -l)
    
    if [ "$OPEN_RULES" -gt 0 ]; then
        echo -e "   ${YELLOW}⚠️ Se encontraron $OPEN_RULES grupos con reglas abiertas a 0.0.0.0/0${NC}"
        echo -e "   ${BLUE}💡 Recomendación: Revisar estas reglas por seguridad${NC}"
    else
        echo -e "   ${GREEN}✅ No se encontraron reglas abiertas a 0.0.0.0/0${NC}"
    fi
fi
...
if [ -z "$ALARMS" ]; then
    echo -e "${RED}❌ No se encontraron CloudWatch Alarms para CIS 3.10${NC}"
else
...
if [ "$SG_COUNT" -gt 0 ]; then
    echo -e "✅ Security Groups: ${GREEN}$SG_COUNT GRUPOS MONITOREADOS${NC}"
else
    echo -e "⚠️ Security Groups: ${YELLOW}NO ENCONTRADOS${NC}"
fi
...
if [ $FILTERS_FOUND -gt 0 ] && [ $ALARMS_FOUND -gt 0 ] && [ -n "$SNS_TOPIC_ARN" ]; then
    echo -e "${GREEN}🎉 CIS 3.10 - CONFIGURACIÓN COMPLETA Y FUNCIONAL${NC}"
    echo -e "${BLUE}💡 Security Group changes monitoring está activo${NC}"
else
    echo -e "${YELLOW}⚠️ CONFIGURACIÓN INCOMPLETA${NC}"
    echo -e "${BLUE}💡 Ejecuta el script de configuración para completar CIS 3.10${NC}"
fi
...
echo "3. Establecer procedimientos de respuesta a cambios de Security Groups"
echo "4. Revisar reglas permisivas existentes (0.0.0.0/0)"
echo "5. Documentar la configuración para el equipo de seguridad"
echo "6. Programar auditorías regulares de Security Groups"