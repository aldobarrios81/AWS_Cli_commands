#!/bin/bash
# setup-config-changes-monitoring.sh
# Create log metric filter and alarm for AWS Config configuration changes
# Regla de seguridad CIS AWS: 3.9 - Monitor AWS Config configuration changes
# Uso: ./setup-config-changes-monitoring.sh [perfil]
...
ALARM_DESCRIPTION="CIS 3.9 - AWS Config configuration changes detected"
NOTIFICATION_EMAIL="felipe.castillo@azlogica.com"

# Patrón del filtro para detectar cambios en AWS Config
...
echo "=================================================================="
echo -e "${BLUE}🔒 IMPLEMENTANDO CIS 3.9 - AWS CONFIG CHANGES MONITORING${NC}"
echo "=================================================================="
echo -e "Perfil: ${GREEN}$PROFILE${NC} | Región: ${GREEN}$REGION${NC}"
echo "Regla: Create log metric filter and alarm for AWS Config configuration changes"
echo ""
...
# Paso 2: Verificar AWS Config
echo -e "${PURPLE}=== Paso 2: Verificando AWS Config ===🎯${NC}"

# Verificar estado de AWS Config
...
if [ -z "$CONFIG_RECORDERS" ]; then
    echo -e "${YELLOW}⚠️ No se encontraron Configuration Recorders en AWS Config${NC}"
    echo -e "${BLUE}💡 AWS Config debe estar habilitado para monitorear cambios${NC}"
else
    echo -e "✅ Configuration Recorders encontrados: ${GREEN}$CONFIG_RECORDERS${NC}"
    
    # Verificar estado de los recorders
    for recorder in $CONFIG_RECORDERS; do
        ...
        if [ "$RECORDER_STATUS" == "true" ]; then
            echo -e "   📊 Recorder '$recorder': ${GREEN}ACTIVO${NC}"
        else
            echo -e "   📊 Recorder '$recorder': ${YELLOW}INACTIVO${NC}"
        fi
    done
fi
...
if [ -z "$DELIVERY_CHANNELS" ]; then
    echo -e "${YELLOW}⚠️ No se encontraron Delivery Channels configurados${NC}"
else
    echo -e "✅ Delivery Channels encontrados: ${GREEN}$DELIVERY_CHANNELS${NC}"
fi
...
# Resumen final
echo "=================================================================="
echo -e "${GREEN}🎉 IMPLEMENTACIÓN CIS 3.9 COMPLETADA${NC}"
echo "=================================================================="
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "✅ Metric Filters para detectar cambios en AWS Config"
echo "✅ CloudWatch Alarms para alertas de modificaciones de configuración"
echo "✅ SNS Topic para notificaciones inmediatas de seguridad"
echo "✅ Cumplimiento con CIS AWS Benchmark 3.9"
echo ""

echo -e "${BLUE}🔍 EVENTOS AWS CONFIG MONITOREADOS:${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "• StopConfigurationRecorder - Detención de grabación de configuraciones"
echo "• DeleteDeliveryChannel - Eliminación de canales de entrega"
echo "• PutDeliveryChannel - Modificación de canales de entrega"
echo "• PutConfigurationRecorder - Modificación de grabadores de configuración"
echo ""

echo -e "${RED}⚠️ IMPORTANCIA DE ESTA REGLA:${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "• AWS Config es crucial para el cumplimiento y auditoría"
echo "• Cambios no autorizados pueden deshabilitar el monitoreo de recursos"
echo "• La eliminación de delivery channels puede interrumpir reportes de cumplimiento"
echo "• Modificaciones maliciosas pueden ocultar actividad sospechosa"
echo "• Detección temprana previene pérdida de visibilidad de configuración"
echo ""
...
echo -e "   ${BLUE}./verify-config-changes-monitoring.sh${NC}"
echo ""
echo "3. Probar las notificaciones (opcional):"
echo -e "   ${BLUE}aws sns publish --topic-arn $SNS_TOPIC_ARN --message 'Prueba de notificación CIS 3.9' --profile $PROFILE${NC}"
echo ""
echo "4. Establecer procedimientos de respuesta para eventos AWS Config:"
echo "   - Investigar inmediatamente cambios no autorizados en configuración"
echo "   - Revisar si se han deshabilitado grabadores de configuración"
echo "   - Validar modificaciones de delivery channels con el equipo de seguridad"
echo ""
echo "5. Monitorear regularmente las alertas y el estado de AWS Config"
echo ""
echo "6. Documentar esta configuración en la política de gobernanza de configuración"